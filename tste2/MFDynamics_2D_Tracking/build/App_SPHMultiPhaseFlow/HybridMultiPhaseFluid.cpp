...
void HybridMultiPhaseFluid::UpdatePhi(Grid<3, T>& d, Grid<3, T>& d0, float dt)
{
	...
	float gamma = 1.0f;
	float ceo1 = 1.0f*gamma / h;
	float w = 1.0f*h;//fro diffuse parameter


	cudaHybridMultiPhaseFluid Update_phi;
	Update_phi.cudaUpdatePhi(d.data, d0.data, dt, Nx, Ny, Nz, h);
	d0 = d;
...
//
//	//cudaHybridMultiPhaseFluid Update_phi;
//	//Update_phi.cudaUpdatePhi((float3*)nGrid.data, dif_field.data, Nx, Ny, Nz);
//
//	float w = 1.0f*h;//fro diffuse parameter
//	float gamma = 1.0f;
...
void HybridMultiPhaseFluid::LinearSolve(Grid3f& d, Grid3f& d0, float a, float c)
{
	...
	cudaHybridMultiPhaseFluid Linear_Solve;
	Grid3f cp;
	...
	for (int it = 0; it < 20; it++)
	{
		...
		cp = d;
		Linear_Solve.cudaLinerSolve(d.data, d0.data, cp.data, c, nx, ny, nz);
	}
}
...
void HybridMultiPhaseFluid::SetScalarFieldBoundary(Grid3f& field, bool postive)
{
	...
	cudaHybridMultiPhaseFluid	Set_ScalarFieldBoundary;
	int nx = field.Nx();
	int ny = field.Ny();
	int nz = field.Nz();

	float s = postive ? 1.0f : -1.0f;
	Set_ScalarFieldBoundary.cudaSetScalarFieldBoundary(field.data, s, nx, ny, nz);
...
void HybridMultiPhaseFluid::AdvectForward(Grid<3, T>& d, Grid<3, T>& d0, GridV3f& v, float dt)
{
	...
	Advect_Forward.cudaAdvectForward(d.data, d0.data, (float3*)v.data, nx, ny, nz, dt, h, dsize);

...
public:
	...
	{
		Vec result;
		for (i = 0; i < m; i++) result[i] = x[i]+v.x[i];
		return result;
	}
...
public:
	...
	Vec<3,T>& operator = (const Vec<2,T_>& other)
	{
		x = (T)other.x;
		y = (T)other.y;
		z = 0;
